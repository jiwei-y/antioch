...
(define-module (ac packages ncurses)
     ...
     (let ((patch-makefile-phase
           ...
           (apply-rollup-patch-phase
            ...
            #~(lambda* (#:key inputs native-inputs #:allow-other-keys)
                (let ((rollup-patch #$(ncurses-rollup-patch-5
                           ...
                           (with-directory-excursion (string-append out "/bin")
                             (for-each
                              (lambda (lib)
                                ...
                                (define libw5.dll
                                  (string-append "lib" lib "w5.dll"))

                                (when (file-exists? libw5.dll)
                                  (format #t "creating symlinks for `lib~a'~%" lib)
                                  (symlink libw5.dll lib.dll)))
